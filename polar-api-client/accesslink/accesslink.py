...
class AccessLink(object):
    ...
    def get_exercises(self, access_token):
        params = {'samples': 'true'}
        return self.oauth.get(endpoint="/exercises", access_token=access_token, params=params)
    
    def get_exercise(self, access_token):
        params = {'samples': 'true'}
        return self.oauth.get(endpoint="/exercises/y6dpRO1W", access_token=access_token, params=params)
    ...
    def get_hr(self, access_token):
        date = "2024-01-08"
        return self.oauth.get(endpoint="/users/continuous-heart-rate/" + str(date), access_token= access_token)
